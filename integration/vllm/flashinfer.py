...
@dataclass
class FISpecDecode:
    """Native speculative verification through a graph-safe prefill wrapper."""
...
class FlashInferMetadataBuilder(AttentionMetadataBuilder[FlashInferMetadata]):
    ...
    def __init__(
        ...
        # A separate wrapper keeps real-prefill planning from replacing the
        # speculative verification plan in a mixed batch (PR 41127).
        ...
        native_spec_as_decode = (
            ...
            and not self.attention_config.use_non_causal
        )
        self._init_reorder_batch_threshold(
            1,
            supports_spec_as_decode=supports_spec_as_decode or native_spec_as_decode,
    ...
    def _get_spec_decode_prefill_wrapper(
        ...
        """Keep graph metadata at fixed addresses, keyed by request count."""
        if use_cudagraph:
            wrapper = self._spec_decode_wrappers_cudagraph.get(batch_size)
        else:
    ...
    def build(
        ...
        if num_decodes > 0:
            if decode_with_flashinfer_trtllm_api:
                ...
                if bool((decode_query_lens > 1).any().item()):
                    assert not self.use_dcp
                    assert self.q_data_type_decode == self.q_data_type_prefill
                    query_len = int(decode_query_lens.max().item())
                    uniform_or_padding = (decode_query_lens == query_len) | (
                        decode_query_lens == 0
                    )
                    assert bool(uniform_or_padding.all().item()), (
                        f"non-uniform decode bucket: {decode_query_lens.tolist()}"
                    )
                    real_decode_tokens = int(qo_indptr_cpu[num_decodes].item())
                    if real_decode_tokens != num_decode_tokens and not (
                        use_cudagraph and pure_decode and num_prefill_tokens == 0
                    ):
                        raise RuntimeError(
                            f"num_decode_tokens ({num_decode_tokens}) != "
                            f"qo_indptr[-1] ({real_decode_tokens}) outside FULL decode."
                        )
                    # Padding rows must not enter the prefill wrapper's query slice.
        ...
        if num_decode_tokens > 0:
            ...
            if not decode_with_flashinfer_trtllm_api:
                ...
                if isinstance(attn_metadata.decode, FISpecDecode):
                    assert not use_dcp and not self.is_kvcache_nvfp4
